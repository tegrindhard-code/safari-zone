return function(_p)
	...
	function BattleClient:doTrainerBattle(data)
		...
		if _p.Menu.options.trainerMusic ~= "Random" and not data.musicId then -- Trainer Music Option
			local musicIds = {
				["HGSS"] = 132822517944377,
				["DPP"] = 92008910934056,
				["BW"] = 74057334904194,
				["XY"] = 93381382255290,
				["SM"] = 104690616539848,
			}
			if musicIds[_p.Menu.options.trainerMusic] then
				data.musicId = musicIds[_p.Menu.options.trainerMusic]
			end
		end
		if not data.musicId then -- Random Trainer Music
			local musicIds = {74057334904194,93381382255290,104690616539848,132822517944377,92008910934056}
			local randomMusicId = musicIds[math.random(#musicIds)]
			data.musicId = randomMusicId --1076549083
		end
...
--]]
	...
	function BattleClient:setupScene()
		_p.Overworld:toggleWeather(true, workspace)
		...
		if self.kind ~= 'pvp' and self.kind ~= '2v2' then
			local musicId = self.musicId or {113646487859369, 132335870367385} -- old: {424133853, 424134350}, really old: 282237556
